...
class ScraperAgent
  def initialize
    ...
    @agent.history_added = Proc.new {random_event}
  end
  ...
  def random_event
    ...
    when 4..6
      sleep(rand(0.2..1))
      @agent.get 'https://fetlife.com/requests'
    when 6..10
      sleep(rand(0.2..1))
      @agent.get 'https://fetlife.com/home#everything'
    end
  end
  # log in to site
  # creates a log in
  ...
  def topic_profiles(id, max = 9999, page = 1)
    topic_results = {:results => [], :maxpage => -1}
    loop do
      begin
        profiles_page = @agent.get "https://fetlife.com/fetishes/#{id}/kinksters?page=#{page}"
      rescue StandardError => e
        topic_results[:maxpage] = page
        break
      end
